...
export const Controls: FC<TimelineExtraControls<Actions, DataType>> = ({ onAction }) => {
  ...
  return (
    <>
      <ControlButton onClick={onKeypointToggle} disabled={!hasSelectedRegion || readonly} tooltip="切换关键帧">
        {keypointIcon}
      </ControlButton>

      <ControlButton onClick={onLifespanToggle} disabled={!closestKeypoint || readonly} tooltip="切换插值">
        {interpolationIcon}
      </ControlButton>
    </>
  );
